...
export default function AgentCard({ agent }: Props) {
    return (
        <div className="bg-[#1E1E1E] border border-[#2C2C2E] rounded-2xl p-6 hover:border-[#3C3C3E] transition-all duration-300 hover:-translate-y-0.5 group" dir="rtl">
            <div className="flex items-start gap-4 mb-5">
                <div className="relative shrink-0">
                    <img src={agent.avatar} alt={agent.name} className="w-14 h-14 rounded-xl object-cover" />
                    {agent.verified && (
                        <div className="absolute -bottom-1 -end-1 w-5 h-5 bg-[#00E5FF] rounded-full flex items-center justify-center">
            ...
            <div className="flex items-center justify-between py-3 border-t border-[#2C2C2E] mb-4 text-start">
                <div>
                    <p className="text-white font-mono font-semibold text-sm">{agent.listings}</p>
                    <p className="text-[#98989D] text-xs">عقارات نشطة</p>
                </div>
