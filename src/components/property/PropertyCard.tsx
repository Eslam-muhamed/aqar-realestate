...
export default function PropertyCard({ property, className }: Props) {
    ...
    const handleCompare = (e: React.MouseEvent) => {
        ...
        if (success) toast.success("Added to comparison", { description: "View compare page to compare properties." });
        else toast.error("Cannot add", { description: "Maximum 4 properties or already added." });
    };

    const typeLabel = property.type.charAt(0).toUpperCase() + property.type.slice(1);
    const bedsLabel = property.stats.bedrooms === 0 ? "Studio" : `${property.stats.bedrooms}`;
            ...
            <div className="bg-[#1E1E1E] border border-[#2C2C2E] rounded-2xl overflow-hidden transition-all duration-300 hover:-translate-y-1 hover:border-[#3C3C3E] hover:shadow-xl hover:shadow-black/40">
                ...
                <div className="relative overflow-hidden aspect-[4/3]">
                    ...
                    <div className="absolute top-3 right-3 flex flex-col gap-2 opacity-0 group-hover:opacity-100 transition-opacity duration-200">
                        <button onClick={handleFavorite} aria-label="Save to favorites"
                            className={cn("w-8 h-8 rounded-lg flex items-center justify-center transition-colors",
                                fav ? "bg-[#FF453A] text-white" : "bg-[#121212]/80 backdrop-blur-sm text-white hover:bg-[#FF453A]"
                            )}>
                            <Heart size={14} fill={fav ? "currentColor" : "none"} />
                        </button>
                        <button onClick={handleCompare} aria-label="Compare property"
                ...
                <div className="p-5">
                    ...
                    <div className="font-mono text-[#00E5FF] text-lg font-semibold mb-4">
                        ...
                        {property.status === "for-rent" && <span className="text-[#98989D] text-xs font-sans font-normal ml-1">/yr</span>}
                    </div>
                    ...
                    <div className="flex items-center gap-4 pt-4 border-t border-[#2C2C2E]">
                        <div className="flex items-center gap-1.5 text-[#98989D] text-xs">
                            <BedDouble size={13} />
                            <span>{bedsLabel} {bedsLabel !== "Studio" ? "Beds" : ""}</span>
                        </div>
                        <div className="flex items-center gap-1.5 text-[#98989D] text-xs">
                            <Bath size={13} />
                            <span>{property.stats.bathrooms} Baths</span>
                        </div>
                        <div className="flex items-center gap-1.5 text-[#98989D] text-xs">
                            <Square size={13} />
