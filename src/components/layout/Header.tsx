...
const NAV_LINKS = [
    { label: "Buy", href: "/properties?status=for-sale" },
            ...
            <div className="max-w-[1440px] mx-auto px-6 lg:px-12">
                <div className="flex items-center justify-between h-16">
                    ...
                    <Link to="/" className="flex items-center gap-2 shrink-0">
                        ...
                        <span className="text-white font-semibold text-lg tracking-tight">Aqar</span>
                    </Link>
                    ...
                    <div className="hidden md:flex items-center gap-2">
                        <Link to="/properties" className="p-2 text-[#98989D] hover:text-white transition-colors rounded-md" aria-label="Search">
                            <Search size={18} />
                        </Link>
                        <Link to="/favorites" className="relative p-2 text-[#98989D] hover:text-white transition-colors rounded-md" aria-label="Favorites">
                            <Heart size={18} />
                            {favorites.length > 0 && (
                                <span className="absolute -top-0.5 -right-0.5 w-4 h-4 bg-[#00E5FF] text-[#121212] rounded-full text-[10px] font-bold flex items-center justify-center">
                                    {favorites.length}
                                </span>
                            )}
                        </Link>
                        {user ? (
                            <div className="relative">
                                <button onClick={() => setUserMenuOpen(!userMenuOpen)}
                                    className="flex items-center gap-2 pl-3 pr-2 py-1.5 rounded-md border border-[#2C2C2E] text-sm text-white hover:border-[#00E5FF]/40 transition-colors">
                                    <User size={14} className="text-[#00E5FF]" />
                                    <span className="max-w-[100px] truncate">{user.name.split(" ")[0]}</span>
                                    <ChevronDown size={12} className="text-[#98989D]" />
                                </button>
                                {userMenuOpen && (
                                    <div className="absolute right-0 top-full mt-2 w-48 bg-[#1E1E1E] border border-[#2C2C2E] rounded-lg py-1 shadow-xl z-50">
                    ...
                    {/* Mobile Menu Button */}
                    <button onClick={() => setMobileOpen(!mobileOpen)} className="md:hidden p-2 text-[#98989D]" aria-label="Menu">
            ...
            {mobileOpen && (
                <div className="md:hidden bg-[#121212] border-t border-[#2C2C2E] px-6 py-4 space-y-1">
                    ...
                    <div className="pt-4 flex gap-3">
                        {user ? (
                            <button onClick={handleLogout} className="flex-1 py-2.5 border border-[#2C2C2E] text-sm text-[#98989D] rounded-md">Sign Out</button>
                        ) : (
                            <Link to="/login" className="flex-1 py-2.5 border border-[#2C2C2E] text-sm text-center text-[#98989D] rounded-md">Sign In</Link>
                        )}
